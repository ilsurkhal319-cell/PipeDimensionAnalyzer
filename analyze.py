...
ТАМ ГДЕ НЕТ СИНЕЙ ЛИНИИ branch начинаться НЕ МОЖЕТ!!!
...
Определяй nested независимо от роли main/branch. Сначала сравни физические границы
размерных отрезков по их геометрии, а не их роль и не числовое значение.

Размер A является nested относительно B только если оба относятся к одному и тому же
участку осевой трассы, обе границы A полностью находятся внутри границ B, а B покрывает
весь физический интервал A. Если совпадает только одна точка, размеры соседние, а не
nested. Если границы нельзя уверенно сопоставить по геометрии, оставь оба размера
включёнными и поставь status=review.

Если размеры имеют только одну общую конечную точку или их физические интервалы
на осевой трассе не совпадают и один не покрывает другой полностью, они не nested.
