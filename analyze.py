...
SYSTEM_PROMPT = """
Ты анализируешь российский изометрический чертёж трубопровода. Выбери набор размеров, который
один раз и без перекрытий покрывает всю трассу на листе, включая ответвления.

Геометрический этап уже выполнил поиск размеров и удалил служебные числа. Поэтому не ищи новые
кандидаты и не исключай переданные C-ID как служебные. Твоя задача только: определить main/branch,
исключить доказанно вложенные размеры.
...
- Поле dimension_line содержит нормализованные координаты концов красного размерного отрезка.
  Используй их для проверки направления, границ и вложенности размеров.

Твоя задача только определить кандидат является main branch или nested. 

ОСНОВНЫЕ ПРАВИЛА:

branch:
Тройник — узел, в котором сходятся три осевые линии трубы. В каждом тройнике ровно один
из трёх выходов является branch, а два других образуют непрерывную основную трассу и являются
main. Сначала определи пару выходов, соединённых одной трассой через узел; оставшийся
третий выход обязательно назначь branch. После этого все последующие участки той же
непрерывной осевой ветви до конечной точки также являются branch. Не определяй роль по длине,
углу или положению числа.

Алгоритм для каждого узла с тремя осевыми выходами: пронумеруй выходы A, B и C и проследи
каждый из них наружу до следующего узла, подключения или конца листа. Сравни пары A-B, A-C и
B-C. Main является та пара, чьи осевые линии соединяются через узел без разрыва и продолжают
одну трассу; оставшийся третий выход обязательно branch. Поворот одной трубы не разрывает main.
Не выбирай main по длине участка, размеру DN или близости числа.

Различай строго: настоящий тройник — внутренний узел трубы с тремя осевыми выходами и одним
боковым branch; конечное подключение — конец трассы, branch там не создаётся; пересечение
размерных линий или выносок не является узлом трубы и branch не создаёт.
Определяй узлы и количество branch только по синей подсветке DN…X… и связанным с ней осевым
линиям. Остальные места на чертеже не используй для поиска тройников.

На узле пересечения/тройнике используй синюю подсветку DN только как подсказку числа выходов:
один символ X означает один боковой branch-выход, два символа X — два боковых branch-выхода.
Применяй это правило только в месте пересечения осевых линий; на одиночном конце трассы по
числу X branch не назначай.

Синюю линию на самом конце или начале трубы игнорируй, если там нет внутреннего тройника.
Branch может начинаться только от подтверждённого синего узла; соседние размеры сами по себе
branch не являются.

nested:

- Сначала включи все размеченные размеры.
- Ищи параллельные размерные линии, относящиеся к одной непрерывной осевой трассе.
- Если длинная линия полностью охватывает физический участок одной или нескольких коротких линий, выбери длинный общий размер. Короткие размеры внутри неё пометь nested и исключи.
- Длинный общий размер нельзя помечать nested относительно покрываемых им коротких размеров.
- Параллельность, меньшее число, близость или одна общая конечная точка сами по себе не доказывают вложенность.
- Размеры разных ответвлений и соседних неперекрывающихся участков не являются nested.
- Числовые значения и их суммы при определении вложенности не используй.
- Для каждого nested укажи в reason один покрывающий C-ID.

Размер A является nested относительно B только если:
1. оба размера относятся к одной и той же осевой трассе;
2. границы A полностью находятся между границами B;
3. обе границы A совпадают с внутренними точками участка B.

Если размеры имеют только одну общую точку или относятся к разным выходам/ветвям, они не nested.

Для каждого кандидата верни candidate_id, segment_id, value_mm, role и included. У всех размеров
одной связной ветви роли должны быть согласованы. Не вычисляй сумму: это сделает программа.
