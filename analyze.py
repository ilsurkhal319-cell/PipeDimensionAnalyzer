...
2. Используй геометрию красных размерных отрезков и поле dimension_line. Число value_mm
   используй только как значение результата, но не как доказательство вложенности.
3. Для каждой пары проверь, относятся ли они к одному физическому участку одной осевой
   трассы. Если да, сравни их физические границы вдоль трассы.
4. Размер A пометь nested относительно B только если обе границы A находятся внутри
   границ B, а B полностью покрывает тот же физический интервал. B — внешний общий размер,
...
role=main и included=true. Верни только структурированный объект без Markdown.
""".strip()
